    ...
    <div class="container mx-auto grid lg:grid-cols-7 grid-cols-1 gap-12">
        ...
        <div class="lg:col-span-5 col-span-7">
            <div class="bg-base-100 shadow-xl p-8 rounded-2xl mb-5">
                ...
                <form class="card-body" method="post" action="{{route('update.news-event',$newsevent->id)}}" enctype="multipart/form-data" >
                    @csrf
                    @method('patch')

                    <div class="avatar">
                      <div class="w-full h-96 border">
                        <img id="image-preview" class="object-cover" src="{{asset('assets/image/event-news')}}/{{$newsevent->thumbnail}}" type="file" accept="image/*" />
                      </div>
                    </div>

                    <input id="avatarInput" type="file" class="file-input file-input-bordered file-input-primary w-full max-w-xs" name="thumbnail" accept="image/*" onchange="previewImage(event)"/>
        
                  <div class="form-control">
                    <label class="label" for="title">
                        <span class="label-text">Title</span>
                    </label>
                    <input type="text" id="title" placeholder="Name" value="{{$newsevent->title}}" class="input input-bordered" name="title" autofocus autocomplete="name" />
                  </div>
                  ...
                  <div class="form-control">
                    ...
                    <select class="select select-bordered w-full" name="type" id="type">
                        <option value="News" @if($newsevent->type == "News") selected @endif>News</option>
                        <option value="Event" @if($newsevent->type == "Event") selected @endif>Event</option>
                    </select>
                  </div>
                  ...
                  <div class="form-control">
                    ...
                    <textarea class="textarea textarea-bordered" name="details" placeholder="Add details" id="details">
                        {{$newsevent->details}}
                    </textarea>
                  </div>
